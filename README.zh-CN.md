...
意图路由会先判断用户消息是否需要 SQL。业务数据问题进入 Schema/知识检索、SQL 生成、SQL 校验、行级权限注入、查询执行、图表推断和洞察建议链路。固定报表类请求可以跳过 SQL 生成，直接运行已配置的工作流。定义解释、澄清、闲聊和搜索类问题会在 Text-to-SQL 前提前返回，不生成 SQL，也不会查询业务数据源。
...
访问 [http://localhost:3000](http://localhost:3000)。详见 [docs/deployment.zh-CN.md](./docs/deployment.zh-CN.md)。英文版见 [docs/deployment.md](./docs/deployment.md)。
