-- ChatBI demo datasource for PostgreSQL.
--
-- Load into a dedicated database:
--   createdb chatbi_demo
--   psql -d chatbi_demo -f data/demo-postgres.sql
--
-- ChatBI's current schema sync reads the public schema, so these objects are
-- intentionally created in public.
...
GRANT CONNECT ON DATABASE chatbi_demo TO chatbi_demo_readonly;
GRANT USAGE ON SCHEMA public TO chatbi_demo_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO chatbi_demo_readonly;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO chatbi_demo_readonly;
